...
for subject in "${subjects[@]}"; do
    for hemisphere in ${hemispheres}; do
        ...
        nohup matlab -nodisplay -nosplash -nodesktop -r "vtk_file='${vtk_file}'; data_to_recon='${data_to_recon}'; eigen_file='${eigen_file}';reconstruction_file='$reconstruction_file'; data_output='$output_dir_plots';hemisphere ='$hemisphere'; num_modes=$num_modes; map_int=$map_int;medial_wall_file='$medial_wall_file'; mode_interest=${arr_in_matlab};dir_func='${dir_func}'; plot_brains_tau(vtk_file, data_to_recon,eigen_file, reconstruction_file, medial_wall_file, data_output, hemisphere, num_modes, map_int,mode_interest, dir_func);" > "${log_file}" 2>&1  &
